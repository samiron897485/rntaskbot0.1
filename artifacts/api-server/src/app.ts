...
app.get("/task", (_req, res) => {
  try {
    ...
    html = html.replace("<!-- ADSTERRA_BANNER_468_INTRO -->", banner468);
    ...
    // Interstitial ad: when admin enables it AND has pasted code, inject
    // that code (typically a Mondiad/Adsterra interstitial <script> tag)
    // directly into the page head. These networks' interstitial scripts
    // hook into user clicks themselves and show their own full-screen ad
    // overlay — we do not need to manage container/timing on our side.
    const interstitialEnabled = cfg.interstitialAdEnabled === true && (cfg.interstitialAdCode || "").trim() !== "";
    const interstitialScript = interstitialEnabled ? (cfg.interstitialAdCode || "") : "";
    html = html.replace("<!-- INTERSTITIAL_AD_SCRIPT -->", interstitialScript);
